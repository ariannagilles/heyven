...
export async function getOwnPosts(
  ...
  let query = supabase
    .from("posts")
    .select("id, space_slug, content, created_at, edited_at, replies(count), me_too(count)")
    .eq("author_id", userId)
...
export async function getOwnQuestions(
  ...
  let query = supabase
    .from("questions")
    .select("id, space_slug, content, created_at, edited_at, question_replies(count)")
    .eq("author_id", userId)
    .order("created_at", { ascending: false })
    .order("id", { ascending: false })
    .limit(limit + 1);
...
export async function getOwnStories(
  ...
  let query = supabase
    .from("stories")
    .select("id, space_slug, title, content, created_at, edited_at, story_reactions(count)")
    .eq("author_id", userId)
    .order("created_at", { ascending: false })
    .order("id", { ascending: false })
    .limit(limit + 1);
